<?php
        if(isset($_GET["ogrenci"]) ){
				...
				$query = "SELECT studentID FROM etumobile WHERE studentID= '".$id."' LIMIT 1";
				...
				if($result->num_rows!="1"){
					...
					//echo "Pulled from DB";
					$query = "SELECT programme FROM etumobile WHERE studentID='".$id."' LIMIT 1";
					
					$homepage = $connection->query($query);
					
					$homepage = $homepage->fetch_object();
					
					echo($homepage->programme);
				}
